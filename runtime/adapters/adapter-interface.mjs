...
export function assertAllAdaptersConform(adapters = {}) {
  ...
  for (const [key, adapter] of Object.entries(adapters)) {
    checked += 1;
    // Alias keys (ember → ember-protocol, tramplin → tramplin.io, etc.) share one
    // adapter object whose canonical venueId is the primary id. Only enforce
    // key===venueId when the key is the canonical venueId.
    const primary = String(adapter?.venueId || "").toLowerCase();
    const keyL = String(key || "").toLowerCase();
    const r = assertAdapterConforms(
      adapter,
      primary && keyL === primary ? { expectedVenueId: key } : {},
    );
    if (!r.ok) {
