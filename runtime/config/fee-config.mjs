 ...
 *
 * LIVE charges today (mav 2026-07-25):
 *   - Swap 0.10% (10 bps) — configurable
 *   - Bridge 0.10% (10 bps) — configurable
 *   - Gas sponsorship 0.10% (10 bps) — configurable
...
/**
 * Gas sponsorship fee typical mid (0.10%); range 5–30 bps product rail.
 * LIVE product charge (mav 2026-07-25) — same family as swap/bridge.
 * On-chain DayRegistry still allows up to 1500 bps owner override;
 * MAX retained at 1500 so on-chain reads do not fail closed incorrectly.
 * Historical mid was 10% of gas (1000 bps) — superseded as the product default.
 */
export const DEFAULT_PROTOCOL_GAS_SPONSOR_FEE_BPS = 10;
export const DEFAULT_GAS_SPONSOR_MIN_BPS = 5;
export const DEFAULT_GAS_SPONSOR_MAX_BPS = 1500;
/** @deprecated historical gas-sponsor mid (10% of gas); product default is 10 bps. */
...
export const GLOBAL_FEE_DEFAULTS = Object.freeze({
  ...
  gas_sponsor_markup: Object.freeze({
    ...
    charged_by: "DAY",
    notes: "0.10_pct_default_configurable_with_swap_bridge",
  }),
  /**
...
/**
 * Gas-sponsor fee bps. Prefer on-chain protocolGasSponsorFeeBps when
 * `ctx.onChainGasSponsorFeeBps` is provided. Product default 10 (0.10%).
...
export function getGasSponsorFeeBps(ctx = {}) {
  if (ctx.onChainGasSponsorFeeBps != null) {
    ...
    if (Number.isSafeInteger(n) && n >= 0 && n <= DEFAULT_GAS_SPONSOR_MAX_BPS) {
      // On-chain owner may set 0..max (including temporarily off at 0).
      return n;
    }
  }
...
export function assertFeeConfigLocked() {
  ...
  // Live DAY rail charges: swap + bridge + gas-sponsor at 10 bps each (0.10%).
  ...
  if (DEFAULT_PROTOCOL_GAS_SPONSOR_FEE_BPS !== 10) {
    throw Object.assign(
      new Error(
        `DEFAULT_PROTOCOL_GAS_SPONSOR_FEE_BPS must be 10 (0.10%); got ${DEFAULT_PROTOCOL_GAS_SPONSOR_FEE_BPS}`,
      ),
      { code: "FEE_CONSTANT_DRIFT", status: 500 },
    );
  }
  ...
  if (getSwapFeeBps() !== 10 || getBridgeFeeBps() !== 10 || getGasSponsorFeeBps() !== 10) {
    throw Object.assign(
      new Error(
        "swap, bridge, and gas-sponsor must each resolve to 10 bps (0.10% live rails)",
      ),
      { code: "FEE_CONSTANT_DRIFT", status: 500 },
    );
  }
...
export function dayFeeSchedule(ctx = {}) {
  ...
  return {
    ...
    notes: [
      ...
      "live_charges_swap_bridge_gas_sponsor_10_bps_each",
